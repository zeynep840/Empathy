...
public class HayvanYorumActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_hayvan_yorum);
        Post_Key4 = getIntent().getExtras().get("PostKey4").toString();
        postsRef2 = FirebaseDatabase.getInstance().getReference().child("Hayvan").child(Post_Key4).child("Comments");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText(HayvanYorumActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef2.child(postRandom2).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(HayvanYorumActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(HayvanYorumActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<HayvanYorum,HayvanYorumViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<HayvanYorum, HayvanYorumViewHolder>
                (HayvanYorum.class,
                        R.layout.all_comments2_layout,
                        HayvanYorumViewHolder.class,
                        postsRef2

                )
        {
            @Override
            protected void populateViewHolder(HayvanYorumViewHolder viewHolder, HayvanYorum model, int position) {
    ...
    public static class HayvanYorumViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public HayvanYorumViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak

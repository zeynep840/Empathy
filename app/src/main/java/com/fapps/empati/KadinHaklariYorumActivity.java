...
public class KadinHaklariYorumActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_kadin_haklari_yorum);
        Post_Key3 = getIntent().getExtras().get("PostKey3").toString();
        postsRef1 = FirebaseDatabase.getInstance().getReference().child("KadinHaklari").child(Post_Key3).child("KadinHaklariYorum");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText( KadinHaklariYorumActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef1.child(postRandom1).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(KadinHaklariYorumActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(KadinHaklariYorumActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<KadinHaklariYorum,KadinHaklariYorumViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<KadinHaklariYorum, KadinHaklariYorumViewHolder>
                (KadinHaklariYorum.class,
                        R.layout.all_comments1_layout,
                       KadinHaklariYorumViewHolder.class,
                        postsRef1

                ) {
            @Override
            protected void populateViewHolder(KadinHaklariYorumViewHolder viewHolder, KadinHaklariYorum model, int position) {
    ...
    public static class KadinHaklariYorumViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public KadinHaklariYorumViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak

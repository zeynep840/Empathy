...
public class SucYorumActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_suc_yorum);
        Post_Key4 = getIntent().getExtras().get("PostKey4").toString();
        postsRef2 = FirebaseDatabase.getInstance().getReference().child("Suc").child(Post_Key4).child("Comments");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText(SucYorumActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef2.child(postRandom2).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(SucYorumActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(SucYorumActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<SucYorum,SucYorumViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<SucYorum, SucYorumViewHolder>
                (SucYorum.class,
                        R.layout.all_comments2_layout,
                        SucYorumViewHolder.class,
                        postsRef2

                )
        {
            @Override
            protected void populateViewHolder(SucYorumViewHolder viewHolder, SucYorum model, int position) {

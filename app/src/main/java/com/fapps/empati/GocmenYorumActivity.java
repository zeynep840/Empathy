...
public class GocmenYorumActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_gocmen_yorum);
        Post_Key = getIntent().getExtras().get("PostKey2").toString();
        postsRef = FirebaseDatabase.getInstance().getReference().child("Gocmen").child(Post_Key).child("Comments");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText(GocmenYorumActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef.child(postRandom).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(GocmenYorumActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(GocmenYorumActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<GocmenYorum,GocmenYorumViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<GocmenYorum, GocmenYorumViewHolder>
                (GocmenYorum.class,
                        R.layout.all_comments_layout,
                       GocmenYorumViewHolder.class,
                        postsRef

                )
        {
            @Override
            protected void populateViewHolder(GocmenYorumViewHolder viewHolder,GocmenYorum model, int position) {
    ...
    public static class GocmenYorumViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public GocmenYorumViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak

...
public class TeknolojiYorumActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_teknoloji_yorum);
        Post_Key = getIntent().getExtras().get("PostKey2").toString();
        postsRef = FirebaseDatabase.getInstance().getReference().child("Teknoloji").child(Post_Key).child("Comments");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText(TeknolojiYorumActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef.child(postRandom).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(TeknolojiYorumActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(TeknolojiYorumActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<TeknolojiYorum,TeknolojiYorumViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<TeknolojiYorum, TeknolojiYorumViewHolder>
                (TeknolojiYorum.class,
                        R.layout.all_comments_layout,
                        TeknolojiYorumViewHolder.class,
                        postsRef

                )
        {
            @Override
            protected void populateViewHolder(TeknolojiYorumViewHolder viewHolder, TeknolojiYorum model, int position) {
    ...
    public static class TeknolojiYorumViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public TeknolojiYorumViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak

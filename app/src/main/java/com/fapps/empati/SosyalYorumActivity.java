...
public class SosyalYorumActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_sosyal_yorum);
        Post_Key4 = getIntent().getExtras().get("PostKey4").toString();
        postsRef2 = FirebaseDatabase.getInstance().getReference().child("Sosyal").child(Post_Key4).child("Comments");
        mAuth2=FirebaseAuth.getInstance();
        current_user_id2=mAuth2.getCurrentUser().getUid();
        UsersReference2=FirebaseDatabase.getInstance().getReference().child("Users3");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText(SosyalYorumActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef2.child(postRandom2).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(SosyalYorumActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(SosyalYorumActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<SucYorum,SucYorumActivity.SucYorumViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<SucYorum, SucYorumActivity.SucYorumViewHolder>
                (SucYorum.class,
                        R.layout.all_comments2_layout,
                        SucYorumActivity.SucYorumViewHolder.class,
                        postsRef2

                )
        {
            @Override
            protected void populateViewHolder(SucYorumActivity.SucYorumViewHolder viewHolder, SucYorum model, int position) {
    ...
    public static class SucYorumViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public SucYorumViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak

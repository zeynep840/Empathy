...
public class OzelSorunlarYorumActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ozel_sorunlar_yorum);
        Post_Key3 = getIntent().getExtras().get("PostKey3").toString();
        postsRef1 = FirebaseDatabase.getInstance().getReference().child("OzelSorunlar").child(Post_Key3).child("OzelSorunlarYorum");
        mAuth1=FirebaseAuth.getInstance();
        current_user_id1=mAuth1.getCurrentUser().getUid();
        UsersReference1=FirebaseDatabase.getInstance().getReference().child("Users3");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText( OzelSorunlarYorumActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef1.child(postRandom1).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(OzelSorunlarYorumActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(OzelSorunlarYorumActivity.this, "Eror", Toast.LENGTH_SHORT).show();
...
protected void onStart() {
    ...
    super.onStart();
    FirebaseRecyclerAdapter<OzelSorunlarYorum,OzelSorunlarYorumViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<OzelSorunlarYorum, OzelSorunlarYorumViewHolder>
            (OzelSorunlarYorum.class,
                    R.layout.all_comments1_layout,
                    OzelSorunlarYorumViewHolder.class,
                    postsRef1

            ) {
        @Override
        protected void populateViewHolder(OzelSorunlarYorumViewHolder viewHolder, OzelSorunlarYorum model, int position) {
    ...
    public static class OzelSorunlarYorumViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public OzelSorunlarYorumViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak
